...
export default function App() {
  ...
  const handleLoginSubmit = (e) => {
    ...
    const match = registeredUsers.find(
      (u) => u.email.toLowerCase() === queryEmail && u.password === password,
    );
    if (match) {
      setIsLoggedIn(true);
      setUserProfile({
  ...
  const handleRegisterSubmit = (e) => {
    ...
    if (exists) {
      showToast(
        "Email đã được sử dụng. Bạn vui lòng đăng nhập lại nhé!",
        "info",
      );
      return;
    }
  ...
  const runSimulatedScan = (imgUrl, fileName) => {
    ...
    setTimeout(() => {
      ...
      setMatchedProduct(null);
      showToast(
        "Không tìm thấy tệp hình ảnh tương thích với sản phẩm ⚜️",
        "info",
      );
    }, 1900);
  };
    ...
    <div className="min-h-screen bg-white text-gray-800 flex flex-col justify-between selection:bg-[#74070e]/10 selection:text-[#74070e]">
      ...
      <Header
        ...
        onNavigate={(page) => {
          if (page === "account" && !isLoggedIn) {
            ...
            showToast(
              "Quý cô xin vui lòng Đăng nhập để trải nghiệm đặc quyền hội viên nhé! ✨",
              "info",
            );
          } else {
            setCurrentPage(page);
      ...
      <ReviewModal
        ...
        onReviewSubmit={(e) => {
          ...
          showToast(
            "Trân trọng cảm ơn Quý cô đã đánh giá! Ý kiến vàng ngọc này giúp Chéri hoàn thiện không ngừng. 🌹",
            "success",
          );
