    ...
    try {
      ...
      if (data.error) {
        ...
        if (oldStatus && oldStatus !== data.currentStatus) {
          showToast(
            `Cập nhật vận trình ${data.orderId}: Đơn hàng vừa chuyển sang "${data.currentStatusText}" 🚀`,
            "success",
          );
        } else if (!oldStatus) {
          showToast(
            `Đồng bộ vận đơn ${data.orderId}: Trạng thái hiện tại là "${data.currentStatusText}" 📦`,
            "success",
          );
        } else {
          showToast(
            `Trạng thái đơn hàng ${data.orderId}: "${data.currentStatusText}" ✨`,
            "info",
          );
        }
      ...
      setError(
        "Có lỗi xảy ra khi đồng bộ hành trình. Quý cô vui lòng thử lại sau.",
      );
      setTrackingData(null);
    } finally {
      setLoading(false);
          ...
          <div className="bg-white border border-gray-100 p-6 rounded-lg shadow-sm grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
            ...
            <div className="space-y-1 md:border-l md:border-r md:px-6 border-gray-100">
              ...
              <h4 className="text-[13px] font-medium text-gray-800 flex items-center">
                <span className="mr-1">{trackingData.carrierLogo}</span>{" "}
      ...
      {!trackingData && !loading && !error && (
        <div className="text-center py-12 md:py-16 bg-[#FCFBF9]/60 border border-dashed border-gray-200 rounded-lg max-w-xl mx-auto">
          ...
          <p className="text-xs text-gray-400 max-w-sm mx-auto mt-2 leading-relaxed">
            Nhập Mã Đơn hàng Chéri cao cấp của Quý cô ở phía trên để cập nhật
            tức thì lịch trình và tiến trình vận chuyển theo thời gian thực.
          </p>
        </div>
      )}
    </div>
