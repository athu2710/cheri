...
function mapColorNameToHex(name) {
  ...
  if (norm.includes("xám") || norm.includes("grey") || norm.includes("gray"))
    return "#A9A9A9";
  if (
    norm.includes("nâu") ||
    norm.includes("brown") ||
    ...
    return "#8B5A2B";
  if (norm.includes("vàng") || norm.includes("gold") || norm.includes("yellow"))
    return "#FADA5E";
  if (norm.includes("navy") || norm.includes("xanh navy")) return "#002060";
  if (
    norm.includes("xanh lá") ||
    norm.includes("green") ||
    norm.includes("emerald")
  )
    return "#2E8B57";
  if (
    norm.includes("xanh nhạt") ||
    norm.includes("mint") ||
    norm.includes("xanh lơ")
  )
    return "#E0F2F1";
...
function parseTSV(tsvText) {
  ...
  for (const [groupId, rows] of Object.entries(groups)) {
    ...
    const visibleStr = (getCol(productRow, "visible", 3) || "TRUE")
      .toString()
      .trim()
      .toUpperCase();
    ...
    const categoryPath = (
      getCol(productRow, "categorySlugs", 5) || ""
    ).toLowerCase();

    // Map categories
    let category = "dresses";
    if (
      categoryPath.includes("áo") ||
      categoryPath.includes("top") ||
      categoryPath.includes("shirt") ||
      categoryPath.includes("gile") ||
      categoryPath.includes("corset")
    ) {
      category = "tops";
    } else if (
      categoryPath.includes("quần") ||
      categoryPath.includes("váy") ||
      categoryPath.includes("bottom") ||
      categoryPath.includes("skirt")
    ) {
      category = "bottoms";
    } else if (
      categoryPath.includes("blazer") ||
      categoryPath.includes("tweed") ||
      categoryPath.includes("khoác") ||
      categoryPath.includes("outerwear") ||
      categoryPath.includes("jacket")
    ) {
      category = "outerwear";
    } else if (
      categoryPath.includes("đầm") ||
      categoryPath.includes("dress") ||
      categoryPath.includes("dresses") ||
      categoryPath.includes("tiệc")
    ) {
      category = "dresses";
    } else {
      ...
      if (lowerName.includes("đầm") || lowerName.includes("dress")) {
        ...
        lowerName.includes("áo sơ mi") ||
        lowerName.includes("áo thun") ||
        lowerName.includes("áo gile") ||
        lowerName.includes("áo lụa") ||
        lowerName.includes("áo thắt nơ") ||
        lowerName.includes("áo yếm") ||
        lowerName.includes("top") ||
        lowerName.includes("corset")
      ) {
        category = "tops";
      } else if (
        ...
        lowerName.includes("áo len") ||
        lowerName.includes("áo khoác")
      ) {
        category = "outerwear";
      }
    }
    ...
    const getOptionValues = (optName, optChoices) => {
      ...
      if (
        ...
        lowerName.includes("kích cỡ") ||
        lowerName.includes("kích thước")
      ) {
        choices.forEach((v) => {
          if (v) sizesSet.add(v);
        });
      } else if (
        lowerName.includes("phân loại") ||
        lowerName.includes("loại") ||
        lowerName.includes("subclass") ||
        ...
        lowerName.includes("màu") ||
        lowerName.includes("sắc") ||
        lowerName.includes("col")
      ) {
        choices.forEach((v) => {
          if (!v) return;
          if (v.includes(":")) {
            ...
            if (
              !colorsList.some(
                (col) => col.name.toLowerCase() === v.toLowerCase(),
              )
            ) {
    ...
    if (variantRows.length > 0) {
      const salePrices = variantRows
        .map((r) => parseFloat(cleanPriceString(getCol(r, "price", 11))))
        .filter((p) => !isNaN(p) && p > 0);
      const originalPrices = variantRows
        .map((r) =>
          parseFloat(cleanPriceString(getCol(r, "strikethroughPrice", 12))),
        )
        .filter((p) => !isNaN(p) && p > 0);
      ...
      const stockCounts = variantRows
        .map((r) => parseInt(getCol(r, "inventory", 14) || "0"))
        .filter((s) => !isNaN(s));
        ...
        for (let j = 1; j <= 6; j++) {
          ...
          if (optName && optChoices) {
            ...
            if (
              ...
              if (
                !colorsList.some(
                  (col) => col.name.toLowerCase() === optChoices.toLowerCase(),
                )
              ) {
                colorsList.push({
                  name: optChoices,
                  hex: mapColorNameToHex(optChoices),
                });
              }
            }
          }
    ...
    if (basePrice === 0) {
      basePrice =
        parseFloat(cleanPriceString(getCol(productRow, "price", 11))) || 450000;
    }
    if (!baseOriginalPrice) {
      const orig =
        parseFloat(
          cleanPriceString(getCol(productRow, "strikethroughPrice", 12)),
        ) || 0;
      if (orig > basePrice) {
...
app.get("/api/products", async (req, res) => {
  ...
  try {
    ...
    if (parsed && parsed.length > 0) {
      ...
      console.warn(
        "Spreadsheet parsed to empty array. Falling back to local static database.",
      );
      res.json(CHERI_PRODUCTS);
    }
  } catch (error) {
    console.error(
      "Error loading spreadsheet dynamically, serving fallback static data:",
      error.message || error,
    );
    res.json(CHERI_PRODUCTS);
  }
